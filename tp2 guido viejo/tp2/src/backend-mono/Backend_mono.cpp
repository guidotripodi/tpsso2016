...
vector<vector<char> > tablero_contenido; // tiene conteniedo que aún no son barcos válidas
vector<vector<char> > tablero_barco; // solamente tiene las barcos válidas
...
int main(int argc, const char* argv[]) {
    ...
    tablero_contenido = vector<vector<char> >(alto);
    for (unsigned int i = 0; i < alto; ++i) {
        tablero_contenido[i] = vector<char>(ancho, VACIO);
    }

    tablero_barco = vector<vector<char> >(alto);
    for (unsigned int i = 0; i < alto; ++i) {
        tablero_barco[i] = vector<char>(ancho, VACIO);
    }
...
void atendedor_de_jugador(int socket_fd) {
    ...
    list<Casillero> barco_actual; // lista de letras de la palabra aún no confirmada

    if (recibir_nombre(socket_fd, nombre_jugador) != 0) {
        // el cliente cortó la comunicación, o hubo un error. Cerramos todo.
        terminar_servidor_de_jugador(socket_fd, barco_actual);
    }
    ...
    while (true) {
        ...
        if (comando == MSG_CONTENIDO) {
            ...
            if (es_ficha_valida_en_barco(ficha, barco_actual)) {
                barco_actual.push_back(ficha);
                tablero_contenido[ficha.fila][ficha.columna] = ficha.letra;
                // OK
            ...
            for (list<Casillero>::const_iterator casillero = barco_actual.begin(); casillero != barco_actual.end(); casillero++) {
                tablero_barco[casillero->fila][casillero->columna] = casillero->letra;
            }
            barco_actual.clear();
...
int parsear_casillero(char* mensaje, Casillero& ficha) {
    int contenido = sscanf(mensaje, "contenido %d %d %c", &ficha.fila, &ficha.columna, &ficha.contenido);
...
int enviar_tablero(int socket_fd) {
    ...
    for (unsigned int fila = 0; fila < alto; ++fila) {
        for (unsigned int col = 0; col < ancho; ++col) {
            char contenido = tablero_barco[fila][col];
...
int enviar_error(int socket_fd) {
    char buf[PARTE_BARCOAJE_MAXIMO+1];
    sprintf(buf, "ERROR");
    return enviar(socket_fd, buf);
}
...
void quitar_barcos(list<Casillero>& barco_actual) {
    for (list<Casillero>::const_iterator casillero = barco_actual.begin(); casillero != barco_actual.end(); casillero++) {
        tablero_contenido[casillero->fila][casillero->columna] = VACIO;
    }
    barco_actual.clear();
}
...
bool es_ficha_valida_en_barco(const Casillero& ficha, const list<Casillero>& barco_actual) {
    ...
    if (tablero_contenido[ficha.fila][ficha.columna] != VACIO) {
        return false;
    }
    ...
    if (barco_actual.size() > 0) {
        ...
        if (distancia_vertical == 0) {
            ...
            for (unsigned int columna = mas_distante.columna; columna != ficha.columna; columna += paso) {
                ...
                if (!(puso_contenido_en(ficha.fila, columna, barco_actual)) && tablero_barco[ficha.fila][columna] == VACIO) {
                    return false;
                }
            }
            ...
            for (unsigned int fila = mas_distante.fila; fila != ficha.fila; fila += paso) {
                // el casillero DEBE estar ocupado en el tablero de palabras
                if (!(puso_contenido_en(fila, ficha.columna, barco_actual)) && tablero_barco[fila][ficha.columna] == VACIO) {
                    return false;
                }
            }
        }
...
bool puso_contenido_en(unsigned int fila, unsigned int columna, const list<Casillero>& contenido) {
    for (list<Casillero>::const_iterator casillero = contenido.begin(); casillero != contenido.end(); casillero++) {
