 ...
 *es_ultimo= (pid_ultimo);
 ...
 if (pid_ultimo==pid)
	printf("Proceso con PID %u es ahora el �ltimo.\n", pid);
...
void elector(t_pid pid)
{
 ...
 /* Inicializo generador de n�meros al azar. */
 ...
 while (!fin)
	{
	 ...
	 printf("Comienza la elecci�n de l�der en proceso %u.\n", pid);
	 /* Etapa 2: corro la elecci�n de l�der. */
	 if ((random() % DENOMINADOR_PROB_INICIO)==0)
		{
		 printf("Poniendo a circular una elecci�n en proceso %u.\n",
			pid);
		 iniciar_eleccion(pid, es_ultimo);
		}

	 eleccion_lider(pid, es_ultimo, PERIODO_ELECCION);
	 printf("Finalizada la elecci�n de l�der en proceso %u.\n", pid);
	}
}
